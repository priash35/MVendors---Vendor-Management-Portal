...
<div class="content">
	...
	<div class="card">
		<div class="card-header header-elements-inline">
			<h5 class="card-title">Results for "Brands List"</h5>
			<a href="<?php echo base_url('Category/addbrand');?>"><button type="button" class="btn btn-primary legitRipple">Add New Brand<div class="legitRipple-ripple"></div></button></a>	
		</div>
		<table class="table table-bordered datatable-button-init-basic">
			...
			<tbody>
				...
				<tr>
					...
					<td>
						<?php if($row->brand_pic) {
						?>
							<img src="<?php echo base_url();?>assets/uploads/brand/<?php echo $row->brand_pic ?>"  onError="this.src = '<?php echo base_url().'assets/uploads/no_image.jpg';?>'" id="img" class="avatar img-responsive rounded img_size" alt="avatar" style="height: 30px; width: 60px"; >
							<?php 
							}else{ ?>
					...
					<td><?php echo $row->brand_c_date ?></td>
					<td><?php echo $row->brand_u_date ?></td>
					<td><?php if($row->brand_status == "ACTIVE"){?>
					<a href="<?php echo base_url().'Category/makeBrandAactive/'.$row->brand_id;?>"><span class="badge badge-success">Active</span></a>
					  <?php }else{ ?>
					<a href="<?php echo base_url().'Category/makeBrandInactive/'.$row->brand_id;?>"><span class="badge badge-danger">Inctive</span></a>
					  <?php }?></td>
					  
					<td class="text-center">
						<a href="<?php echo base_url().'Category/update_brand/'.$row->brand_id;?>"><i class="icon-pencil7"></i></a>
						<!-- <a href="<?php //echo base_url().'Category/delete_brand/'.$row->brand_id;?>"onClick="return doconfirm();"><i class="icon-trash"></i></a> -->
					</td>
				</tr>
				<?php  }?>
			</tbody>

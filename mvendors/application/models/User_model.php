...
public function get_vendors_list($enq_id) 		//updated on 10,31 Dec 2018 	//updated on 11 Jan 2018
	{
		$this->db->select('*');
        ...
        $this->db->or_where('t1.res_enq_status','Closed');
        $this->db->group_end('t1.res_id');
        $this->db->order_by('res_enq_updated','DESC');
        $this->db->group_by('res_enq_updated','DESC');
	...
	function get_quot_list($enq_id,$uid)		//updated on 12,27 Dec 2018
	{
		...
		$this->db->group_by('t1.quot_id');
		$query = $this->db->get();
		return $query->result();
	}

	public function get_UserProfile($uid) // change  9 Jan 18 Ganesh
    {
        $this->db->select('*');
        $this->db->from('tbl_user tu');
        $this->db->join('tbl_states t1', 't1.id = tu.ustate');
        $this->db->join('tbl_country t2', 't2.id = tu.ucountry');
        $this->db->join('tbl_city t3', 't3.id = tu.ucity');
...
return $query->result();
	...
	{												//updated  9 Jan 2019	//updated on 16 Jan 2019
		$this->db->select('*');
		$this->db->from('tbl_vendor_quot t1');
		$this->db->join('tbl_enq_master t2','t2.enq_id=t1.quot_enq_id');
		$this->db->join('tbl_enq_atchmnt t4','t4.atch_enq_id = t2.enq_id','left');
   ...
   public function get_ConfInquiry_details($uid)			//21,24,27,31 Dec 2018
    														// 4 , 14 Jan 2019
     {
     	...
     	//$this->db->join('tbl_enq_bill t8','t8.bill_vend_id= t3.res_vend_id AND t8.bill_user_id = t1.enq_user_id AND t8.bill_enq_id = t1.enq_id AND t8.bill_quot_id = t4.quot_id AND t8.bill_po_id = t5.po_id','left');
     ...
     public function get_CompletedEnq_details($uid)			//22,27,31 Dec 2018    2nd JAN 2019	
     {														//14 Jan 2019
     	$this->db->select('*');
     	$this->db->from('tbl_enq_master t1');
     	...
     	$this->db->join('tbl_vbusiness_details t6','t6.bis_vendor_id = t3.res_vend_id');
     	$this->db->join('tbl_eway_bill t7','t7.bill_vendor_id= t3.res_vend_id AND t7.bill_user_id = t1.enq_user_id AND t7.bill_enq_id = t1.enq_id','left');
     	$this->db->join('tbl_user_feed_remark t8','t8.ufeed_enq_id = t1.enq_id AND t8.ufeed_user_id = t1.enq_user_id');
	...
	function get_live_items_list($search_data) {
 		
 		$this->db->select('t1.*,t2.*,t1.category_title as title,t2.sub_category_title as title');
     		$this->db->from('tbl_category t1');
     		$this->db->join('tbl_sub_category t2','t2.sub_category_id = t1.category_id');
		
		$this->db->group_start();
		$this->db->like('t1.category_title', $search_data);
		$this->db->or_like('t2.sub_category_title', $search_data);
	...
	function get_po_details($uid,$quot_id)					//updated on 8 Jan 2019
	{		
 		
 		$this->db->select('*');
     	$this->db->from('tbl_user_po');
     	
		$this->db->where('po_quot_id', $quot_id);
		$this->db->where('po_user_id', $uid);
		
		$this->db->limit(1);
		$this->db->order_by('po_created','DESC');
		
		return $this->db->get()->result();
	}
	
	public function close_inq($enq_id,$uid,$today)     //10 Jan 2019
     {
        $this->db->set('enq_status','Closed');
        $this->db->set('enq_udated',$today);
        $this->db->where('enq_id',$enq_id);
        $result = $this->db->update('tbl_enq_master');
   ...
   public function add_feedback($data)			// 11 Jan 2018
		{
			$res=$this->db->insert('tbl_user_enq_feedback',$data);
			if($res)
			{
				return $res;
			}
	...
	function get_User_Info($id) // 14 Jan 2018 
	{
	  $this->db->select('*'); 
      $this->db->from('tbl_user');
	...
	function get_User_Inq($id)
	{
	   $this->db->select('*'); 
       $this->db->from('tbl_enq_master t1');	
      $this->db->where('t1.enq_user_id',$id);
      return $query=$this->db->get()->result();
	}
	function get_User_Conf_Inq($id)
	{
	  $this->db->select('*'); 
      $this->db->from('tbl_enq_master t1');
      $this->db->where('t1.enq_status','Deal');
	...
	function get_User_Compete_Inq($id)
	{
	  $this->db->select('*'); 
      $this->db->from('tbl_enq_master t1');
      $this->db->where('t1.enq_status','Completed');
    ...
    public function add_feed_remark($data)                     //14 Jan 2019
     {
       $res = $this->db->insert('tbl_user_feed_remark',$data);
        if($res)
			{
				return $res;
			}
      ...
      public function getnotifications($userid)					//15 Jan 2019
     {
     	$this->db->select('*');
     ...
     public function add_notifications($data)                     //15 Jan 2019
     {
       $res = $this->db->insert('tbl_notification',$data);
       return $res;     
     }

     public function get_bill_details()					//15 Jan 2019 
     {
     	$this->db->select('*');
     	$this->db->from('tbl_enq_master t1');
     	$this->db->join('tbl_sub_category t2','t2.sub_category_id = t1.enq_pro_subcat_id OR t2.sub_category_id = t1.enq_ser_subcat_id');
     	...
     	$this->db->join('tbl_user_po t5','t5.po_enq_id = t1.enq_id AND t5.po_user_id = t1.enq_user_id');
     	$this->db->join('tbl_vbusiness_details t6','t6.bis_vendor_id = t3.res_vend_id');
     	$this->db->join('tbl_enq_bill t7','t7.bill_vend_id= t3.res_vend_id AND t7.bill_user_id = t1.enq_user_id AND t7.bill_enq_id = t1.enq_id','left');
     	$this->db->join('tbl_user t8','t8.user_id = t1.enq_user_id');
     	...
     	$this->db->group_start();
     	$this->db->where('t1.enq_status','Deal');
     	$this->db->or_where('t1.enq_status','Completed');
     	$this->db->group_end();
     ...
     public function delete_user_notifications($noti_id)	//16 Jan 2019
     {
     	$this->db->select('*');
     	$this->db->from('tbl_notification');
     ...
     public function get_user_details($id)		//17 Jan 2019
     {
     	$this->db->select('*');
     	$this->db->from('tbl_user');
     	$this->db->where('user_id',$id);
